...
declare
  ...
  v_legacy_contributions numeric;
  v_historical_xirr_count integer;
begin
  ...
  from public.portfolio_legacy_external_flows;

  -- These checks protect the already reconciled historical source data used by
  -- this private portfolio application.
  if v_legacy_count <> 27 then
    raise exception
      'Expected 27 legacy external flows, found %.',
      v_legacy_count;
  end if;

  if abs(v_legacy_contributions - 236000) > 0.00000001 then
    raise exception
      'Expected 236000 PLN of legacy contributions, found %.',
      v_legacy_contributions;
  end if;

  select count(*)
  into v_historical_xirr_count
  from public.portfolio_xirr_snapshots
  where report_run_id is null
    and calculation_version = 'legacy-manual-v1';

  if v_historical_xirr_count <> 4 then
    raise exception
      'Expected 4 legacy manual XIRR snapshots, found %.',
      v_historical_xirr_count;
  end if;
end;
$$;
